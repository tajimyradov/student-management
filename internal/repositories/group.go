...
func (g *GroupRepository) GetGroupByID(id int) (models.Group, error) {
	...
	query := `select id,name,code,year,profession_id from groups where id=$1`
...
func (g *GroupRepository) GetGroups(input models.GroupSearch) (models.GroupsAndPagination, error) {
	...
	if input.ProfessionID != 0 {
		setValues = append(setValues, fmt.Sprintf("profession_id = $%d", argId))
		args = append(args, input.ID)
		argId++
	}
	...
	if argId > 1 || input.Name != "" {
		query = "select id,name,code,year,profession_id from groups where " + queryArgs
	} else {
		query = "select id,name,code,year,profession_id from groups"
	}
