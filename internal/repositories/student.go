...
func (s *StudentRepository) AddStudent(student models.Student) (models.Student, error) {
	query := `insert into students(first_name, last_name, code, gender, username, password, group_id,birth_date) values ($1, $2, $3, $4, $5, $6, $7,$8)`
...
func (s *StudentRepository) GetStudentByID(id int) (models.Student, error) {
	...
	query := `select id,first_name, last_name, code, gender, username, password, group_id,birth_date,image from students where id=$1`
...
func (s *StudentRepository) GetStudents(input models.StudentSearch) (models.StudentsAndPagination, error) {
	...
	argId := 1
	if input.FirstName != "" {
		setValues = append(setValues, fmt.Sprintf("first_name like'%%%s%%'", input.FirstName))
		//args = append(args, input.Name)
		//argId++
	}

	if input.LastName != "" {
		setValues = append(setValues, fmt.Sprintf("last_name like'%%%s%%'", input.LastName))
		//args = append(args, input.Name)
		//argId++
	}
	...
	if input.Username != "" {
		setValues = append(setValues, fmt.Sprintf("username like '%%%s%%'", input.Username))
	}

	if input.BirthDate != "" {
		setValues = append(setValues, fmt.Sprintf("birth_day = $%d", argId))
	...
	if argId > 1 || input.FirstName != "" || input.LastName != "" || input.Username != "" {
		query = "select id,first_name, last_name, code, gender, username, department_id,image from teachers where " + queryArgs
	} else {
		query = "select id,first_name, last_name, code, gender, username, department_id,image from teachers"
	}
