...
func (h *V1) addLesson(c *gin.Context) {
...
func (h *V1) updateLesson(c *gin.Context) {
	var lesson models.Lesson
	if err := c.ShouldBindJSON(&lesson); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
...
func (h *V1) deleteLesson(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("lid"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
...
func (h *V1) getLessonByID(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("lid"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	c.JSON(http.StatusOK, gin.H{"message": "ok", "status": "ok", "lesson": res})
}
...
func (h *V1) getLessons(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "ok", "status": "ok", "lessons": res})
}
