...
func (h *V1) addTime(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "ok", "status": "ok", "time": res})
}

func (h *V1) updateTime(c *gin.Context) {
	var time models.Time
	if err := c.ShouldBindJSON(&time); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
...
func (h *V1) deleteTime(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("tid"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
...
func (h *V1) getTimeByID(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("tid"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
...
func (h *V1) getTimes(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "ok", "status": "ok", "times": res})
}
